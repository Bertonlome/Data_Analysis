...
def plot_aoi_metrics(aoi_metrics, save_as='eye_movement/aoi_analysis.png'):
    ...
    # Define single color for all AoIs (same visual display)
    single_color = '#7FB3D5'  # Light blue
    
    # Plot 1: Gaze Frequency
    bars1 = ax1.bar(range(len(aois)), frequencies, color=single_color, edgecolor='black', linewidth=1.5)
    ...
    bars2 = ax2.bar(range(len(aois)), avg_dwell_times, color=single_color, edgecolor='black', linewidth=1.5)
...
def export_aoi_metrics_to_csv(aoi_metrics, output_path='eye_movement/aoi_metrics.csv'):
    """
    Export AoI metrics to a CSV file for later analysis.
    ...
    # Sort AoIs by name for consistent ordering
    sorted_aois = sorted(aoi_metrics.items(), key=lambda x: x[0])
    
    with open(output_path, 'w', newline='') as f:
        writer = csv.writer(f)
        # Write header
        writer.writerow(['AoI_Name', 'Fixation_Count', 'Percentage', 'Avg_Dwell_Time_s', 'Total_Dwell_Time_s'])
        
        # Write data for each AoI
        for aoi, metrics in sorted_aois:
            writer.writerow([
                aoi,
                metrics['frequency'],
                f"{metrics['percentage']:.2f}",
                f"{metrics['avg_dwell_time']:.6f}",
                f"{metrics['total_dwell_time']:.6f}"
            ])
    
    print(f"AoI metrics exported to '{output_path}'")
...
if __name__ == "__main__":
    ...
    if len(x_coords) > 0:
        ...
        # Print and export AoI statistics
        print_aoi_statistics(aoi_metrics)
        export_aoi_metrics_to_csv(aoi_metrics, 'eye_movement/aoi_metrics.csv')
        
        # Plot AoI metrics
        plot_aoi_metrics(aoi_metrics, 'eye_movement/aoi_analysis.png')
        
        # Create heatmap for cockpit overlay
        print("\nGenerating heatmap...")
        heatmap, xedges, yedges = create_heatmap(x_coords, y_coords, bins=100, sigma=5)
        
        # Create cockpit overlay
        print("Creating cockpit overlay...")
